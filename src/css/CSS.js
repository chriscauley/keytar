...
export default ({ __base="", __default="", __aliases={}, ...variants }) => {
  const root = cssFunc(__base, variants[__default])
  Object.entries(variants).forEach(([key, value]) => root[key] = cssFunc(__base, value))
  if (!variants.base) {
    root.base = cssFunc(__base, "")
  }
  Object.entries(__aliases).forEach(([alias, original]) => root[alias] = root[original])
  return root
}
